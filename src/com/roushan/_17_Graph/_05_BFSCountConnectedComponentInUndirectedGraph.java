...
class Graph4 {
    ...
    // Method to perform BFS traversal for a disconnected graph and count connected components
    public int BFSDis() {
        ...
        for (int i = 0; i < v; i++) {
            if (!visited[i]) {
                BFS(visited, i); // Perform BFS for unvisited vertex
                count++; // Increment count of connected components
            }
        }
        return count;
    }
...
public class _05_BFSCountConnectedComponentInUndirectedGraph {
    public static void main(String[] args) {
        ...
        int count = g.BFSDis(); // Perform BFS traversal for the entire graph
